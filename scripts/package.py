...
def package(output):
    ...
    plugin_entries = [(f"css-art/{path.relative_to(ROOT).as_posix()}", path)
                      for folder in ("skills", ".codex-plugin", "scripts", "tests", "docs", "web", ".github") for path in source_files(ROOT / folder)]
    plugin_entries += [(f"css-art/{name}", ROOT / name) for name in ("LICENSE", "README.md", ".gitignore", ".gitattributes")]
    skill_entries = [(f"css-art/{path.relative_to(SKILL).as_posix()}", path) for path in source_files(SKILL)]
    skill_entries.append(("css-art/LICENSE", ROOT / "LICENSE"))
    results = []
    for kind, entries in (("plugin", plugin_entries), ("skill", skill_entries)):
        path = output / f"css-art-{version}-{kind}.zip"
